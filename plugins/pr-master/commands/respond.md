---
description: Work through a PR's review feedback end-to-end — fetch comments, classify, propose, then (on your go) fix/reply/resolve/summarize.
argument-hint: "[pr-number-or-url]"
---

# Respond to PR review feedback

Work through a PR's review feedback end-to-end. Use `$ARGUMENTS` as the PR (number or URL) if given, else detect it from the current branch. **Human-gated:** present an assessment and stop for the user's `go` before changing anything.
...
5. After explicit user confirmation, use this review-response loop:
   - apply fixes
   - add or extend a test for every item that pushes a code change where one is applicable — defer to the `test-writing` skill (if the project has one) for the how (see the test-coverage rule below)
   - run checks

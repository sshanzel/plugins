...
5. After explicit user confirmation, use this review-response loop:
   - apply fixes
   - add or extend a test for every item that pushes a code change where one is applicable — defer to the `test-writing` skill (bundled with `pr-master`) for the how (see the test-coverage rule below)
   - run checks

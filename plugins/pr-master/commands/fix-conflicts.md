---
description: Resolve the current branch's merge conflicts against its base WITHOUT losing anyone's work — combine both sides, and ask before resolving any genuinely incompatible hunk.
argument-hint: "[base-branch]"
---

# Fix merge conflicts

Resolve the conflicts blocking the current branch from merging. Base = `$ARGUMENTS` if given, else the PR's base (`gh pr view --json baseRefName`), else `origin/main` / `origin/master`.
